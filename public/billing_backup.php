...
$targets = [
    'billing.json' => $storageDir . '/billing.json',
    'billing_history.json' => $storageDir . '/billing_history.json',
    'pppoe_prices.json' => $storageDir . '/pppoe_prices.json',
    'pppoe_contacts.json' => $storageDir . '/pppoe_contacts.json',
];
...
if ($action === 'export') {
    ...
    $filename = 'billing_backup_' . date('Ymd_His') . '.json';
    header('Content-Type: application/json');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $written = [];
    foreach ($targets as $name => $path) {
        if (!array_key_exists($name, $files)) continue;
        $data = $files[$name];
        if (!is_array($data)) {
            http_response_code(400);
